...
proto._toObjectString = function (obj) {
  if (obj.type === 'Identifier') {
    ...
    var str;
    // 特殊处理
    // 1. $xx.length()，如果没有参数的时候，转换为 xx.length，java 的 String 自带 length()，应该转换为 node string .length
    // 2. $xx.length(foo) 如果有参数的时候，转换为 xx.length(foo)
    if (args.length === 0 && callee.property.name === 'length') {
      str = this._toObjectString(callee.object) + '.' + callee.property.name;
    } else {
      str = this._toObjectString(callee.object) + '.' + callee.property.name + '(';
    ...
    return String(obj.value).replace(/['"]/g, '\\$&');
  }
};
